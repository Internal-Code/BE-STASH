...
class User(SQLModel, table=True):
    ...
    updated_at: Optional[datetime] = Field(default=None, unique=True, nullable=True)
    country_id: int = Field(foreign_key="country.id")
    last_login_at: Optional[datetime] = Field(default=None, unique=True, nullable=True)
...
class SendOtp(SQLModel, table=True):
    ...
    otp_code: str = Field()
    expired_at: datetime = Field(default=local_time()+timedelta(minutes=2))
...
class ErrorLog(SQLModel, table=True):
    ...
    id: int = Field(primary_key=True)
    error_at: datetime = Field(default=local_time())
    user_id: int = Field(foreign_key="user.id")
...
class MonthlySchema(SQLModel, table=True):
    ...
    monthly_category: List["MonthlyCategory"] = Relationship(back_populates="monthly_schema")
    transaction_date: List["TransactionDate"] = Relationship(back_populates="monthly_schema")
...
class MonthlyCategory(SQLModel, table=True):
    ...
    transaction_date: List["TransactionDate"] = Relationship(back_populates="monthly_category")
    transaction_detail: List["TransactionDetail"] = Relationship(back_populates="monthly_category")
...
class TransactionDate(SQLModel, table=True):
    ...
    transaction_detail: List["TransactionDetail"] = Relationship(back_populates="transaction_date")
...
class PaymentMethod(SQLModel, table=True):
    ...
    is_deleted: bool = Field(default=False)
    transaction_detail: List["TransactionDetail"] = Relationship(back_populates="payment_method")
...
class TransactionDetail(SQLModel, table=True):
    ...
    is_deleted: bool = Field(default=False)
    transaction_date: TransactionDate = Relationship(back_populates="transaction_detail")
    monthly_category: MonthlyCategory = Relationship(back_populates="transaction_detail")
