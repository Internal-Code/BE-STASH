...
async def wrong_account_endpoint(
    ...
    logging.info(
        f"[WRONG_ACCOUNT] Incoming request | user_uid={schema.user_uid}, channel={schema.channel}, country_id={schema.country_id}, ip={ip_address}"
    )
    ...
    try:
        ...
        match schema.channel.value:
            case "whatsapp":
                ...
                logging.info(
                    f"[WRONG_ACCOUNT] Phone correction request | user_uid={schema.user_uid}, new_phone={country['dial_code']}{schema.phone_number}"
                )
            case _:
                logging.warning(
