...
from src.auth.utils.generator import random_sso_username
...
async def google_sso_auth(request: Request) -> ResponseDefault:
    try:
        ...
        verify_phone_number_unique_id = str(uuid7())
        ...
        try:
            if registered_email is True:
                await save_verify_phone_number_token(
                    phone_number_unique_id=verify_phone_number_unique_id,
                    email=user_info.email,
                )

                response.success = True
                response.message = f"Account {user_info.email} already saved."
                response.data = {"phone_number_id": verify_phone_number_unique_id}
            else:
                await save_google_sso_account(
                    user_uuid=verify_phone_number_unique_id,
                ...
                response.success = True
                response.message = f"Account {user_info.email} successfully created."
                response.data = {"phone_number_id": verify_phone_number_unique_id}
