...
#TODO: See the logic and refactor

async def update_monthly_spend(schema: UpdateCategorySpending, users:Annotated[dict, Depends(get_current_user)]) -> ResponseDefault:
            ...
            async with database_connection().connect() as session:
                try:
                    if category_is_available is False:
                        ...
                        created_category = money_spend_schemas.insert().values(create_category)
                        updated_daily_spend = money_spends.update().where(
                            and_(
                                money_spends.c.spend_day == schema.spend_day,
                                money_spends.c.spend_month == schema.spend_month,
                                money_spends.c.spend_year == schema.spend_year,
                                money_spends.c.category == schema.category,
                                money_spends.c.description == schema.description,
                                money_spends.c.amount == schema.amount,
                            )
                        ).values(
