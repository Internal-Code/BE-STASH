...
async def update_monthly_spend(schema: UpdateCategorySpending, users:Annotated[dict, Depends(get_current_user)]) -> ResponseDefault:
    ...
    if spending_is_available is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Data daily spending on {schema.spend_day}/{schema.spend_month}/{schema.spend_year} with category {schema.category} not found. Please create first.")
    ...
    try:
        ...
        async with database_connection().connect() as session:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Server Error during daily spending creation: {E}.")
            finally:
                await session.close()
