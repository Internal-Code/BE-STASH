            ...
            async with session.begin():
                try:
                    if not is_available:
                        ...
                        create_spend = money_spend.insert().values(prepared_spend)
                        create_category = money_spend_schema.insert().values(prepared_category)
                        ...
                        logging.info(f"Only inserting data into table {money_spend.name}")
                        create_spend = money_spend.insert().values(prepared_spend)
                        await session.execute(create_spend)
