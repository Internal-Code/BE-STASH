...
async def find_record(
    db: AsyncSession, table: type[SQLModel], column: str, value: str
) -> Row | None:
...
async def insert_record(db: AsyncSession, table: type[SQLModel], data: dict) -> None:
    ...
    try:
        ...
        logging.error(f"Failed to insert record in table {table.__name__} with data {data}: {e}")
        await db.rollback()
...
async def update_record(
    ...
    try:
        ...
        logging.error(
            f"Failed to update record in table {table.__name__} with conditions {conditions} and data {data}: {e}"
        )
        await db.rollback()
        raise DatabaseError(detail="Database query error.")
